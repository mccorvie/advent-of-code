...
{
  if( length(s1) == 0 && length( s2 ) > 0 ) return( T )
  if( length( s2 ) == 0 && length( s1 ) > 0 ) return( F )
  if( length( s1 ) == 0 && length( s2  ) == 0) return( NA )
  ...
  if( typeof( v1)  == "double" && typeof( v2 ) == "double" )
  {
    ...
    if( typeof( v1) == "double")  v1 = list( v1 )
    if( typeof( v2 ) == "double") v2 = list( v2 )
    rr <- is_ordered( v1, v2)
    if( !is.na( rr )) return( rr )
  }
